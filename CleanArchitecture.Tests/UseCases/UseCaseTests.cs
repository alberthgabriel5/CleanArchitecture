...
namespace CleanArchitecture.Tests.UseCases
{
    public class UseCaseTests
    {
        ...
        public void CannotRegisterForCourseAlreadyEnrolledIn()
        {
            ...
            // act
            var response = useCase.Handle(new CourseRegistrationRequestMessage(int.MinValue,new List<string> { "IF3150" }));

            // assert
            Assert.False(response.Success);
            Assert.True(response.Errors.Any(e => e == "unable to register for IF3150"));
        }
